...
export const schedule = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const credentials = await ctx.db
            ...
            .withIndex("by_user_platform", q => q.eq("userId", args.userId).eq("platform", product.platform))
            .first();
        ...
        const platformLaunch = await adapter.scheduleLaunch(product.platformId, new Date(args.scheduledAt));
...
export const list = query({
    args: {
        status: v.optional(v.union(v.literal("draft"), v.literal("scheduled"), v.literal("active"), v.literal("completed"), v.literal("failed")))
    },
    handler: async (ctx, args) => {
...
export const updateStatus = mutation({
    args: {
        id: v.id("launches"),
        userId: v.id("users"),
        status: v.union(v.literal("draft"), v.literal("scheduled"), v.literal("active"), v.literal("completed"), v.literal("failed"))
    },
    handler: async (ctx, args) => {
        const launch = await ctx.db.get(args.id);
        if (!launch || launch.userId !== args.userId) {
